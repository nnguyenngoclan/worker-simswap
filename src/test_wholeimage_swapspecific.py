...
def swap_image(source_image, target_image, image, output_path):
    opt = TestOptions().parse()
    opt.pic_a_path = target_image
    opt.pic_b_path = image
    opt.pic_specific_path = source_image
    opt.output_path = output_path
    opt.custom_output = True
    opt.crop_size = 224
    opt.use_mask = True
    opt.name = "people"
...
def __execute(opt: TestOptions = None):
    if opt is None:
        opt = TestOptions().parse()
        opt.custom_output = False
    ...
    if min_value < opt.id_thres:
        ...
        output_path = opt.output_path if opt.custom_output else os.path.join(opt.output_path, 'result_whole_swapspecific.jpg')
